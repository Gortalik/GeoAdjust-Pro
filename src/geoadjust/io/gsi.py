...
class GSIParser(BaseParser):
    ...
    def _extract_point_info(self, line: str) -> Optional[tuple]:
        """
    ...
    def _build_observations(self) -> List[Observation]:
        ...
        for i, point_id in enumerate(sorted_points):
            data = self.points_data[point_id]
            codes = data['codes']
            meas = data['measurements']
            
            # Определяем тип точки
            is_side_shot = '333' in codes and not any(c in codes for c in ['573', '574', '571', '572'])
